        ...
        override fun onMessageReceivedDuringSequence(event: MessageReceivedEvent) {
            when (sequenceNumber) {
                1.toByte() -> when (java.lang.Byte.parseByte(event.message.contentRaw)) {
                    0.toByte() -> {
                        super.channel.sendMessage("Please enter a unique category name.")
                            .queue { message -> super.addMessageToCleaner(message) }
                        sequenceNumber = 2
                    }
                ...
                2.toByte() -> if (newCategoryName == null) {
                    ...
                    super.channel.sendMessage("Please enter how much roles a user can have from this category? Use 0 for unlimited.")
                        .queue { super.addMessageToCleaner(it) }
                } else {
                    synchronized(iAmRolesCategories) {
                        iAmRolesService.addNewCategory(
                            event.guild.idLong,
                            newCategoryName!!,
                            event.message.contentRaw.toInt()
                        )
                    }
                    super.channel.sendMessage(user.asMention + " Successfully added new category.")
                ...
                3.toByte() -> {
                    synchronized(iAmRolesCategories) {
                        iAmRolesService.removeCategory(
                            event.guild.idLong,
                            iAmRolesCategories[event.message.contentRaw.toInt()].categoryId!!
                        )
                    }
                    super.channel.sendMessage(user.asMention + " Successfully removed the category")
                        .queue { message -> message.delete().queueAfter(1, TimeUnit.MINUTES) }
                    super.destroy()
                }
                ...
                6.toByte() -> {
                    ...
                    super.channel.sendMessage("Name successfully changed.")
                        .queue { message -> message.delete().queueAfter(1, TimeUnit.MINUTES) }
                    super.destroy()
                }
                ...
                7.toByte() -> {
                    ...
                    when {
                        ...
                        matchedRoles.isEmpty() -> throw IllegalArgumentException("Could not find any roles with that name")
                            ...
                            when (iAmRolesService.addOrRemoveRole(
                                event.guild.idLong,
                                iAmRolesCategory!!.categoryId!!,
                                roleId
                            )) {
                ...
                8.toByte() -> {
                    ...
                    channel.sendMessage(user.asMention + " Allowed roles successfully changed.")
        ...
        override fun onMessageReceivedDuringSequence(event: MessageReceivedEvent) {
            when (roles) {
                null -> {
                    ...
                    roles = ArrayList(
                        iAmRolesService.getRoleIds(
                            event.guild.idLong,
                            iAmRolesCategories[selectedCategory].categoryId!!
                        )
                    )
                    if (roles!!.isEmpty()) {
                    ...
                    if (!remove) {
                        message.append("You are allowed to select up to ")
                            .append(if (iAmRolesCategories[selectedCategory].allowedRoles > 0) maxAssignableRoles else "unlimited")
                            .append(" role(s) from this category")
                    }
                    if (remove) {
