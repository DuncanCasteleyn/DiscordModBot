    ...
    override fun commandExec(event: MessageReceivedEvent, command: String, arguments: String?) {
        if (!event.isFromType(ChannelType.PRIVATE)) {
            throw UnsupportedOperationException(
                "You are trying to use a command that is intended to be used by direct messaging.\n" +
        ...
        event.author.mutualGuilds.filter {
            it.getMember(event.author).roles.isEmpty() && it.getRolesByName(
        ...
        if (guildList.isEmpty()) {
            event.channel.sendMessage("There were no guilds/servers were this action could be performed.")
                .queue { it.delete().queueAfter(1, TimeUnit.MINUTES) }
        } else {
            val message =
                MessageBuilder().append("You now have the ability to chat in the following guilds/servers without mobile verification:")
